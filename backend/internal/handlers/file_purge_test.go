...
func TestPurge_SpecificTrashedFiles(t *testing.T) {
	...
	handler := NewFileHandler(db, files.NewServiceWithIngest(db, files.IngestDeps{Storage: svc}), svc, nil, nil, nil, nil, nil, nil)
...
func TestPurge_StorageDeleteFailsReturns500(t *testing.T) {
	...
	handler := NewFileHandler(db, files.NewServiceWithIngest(db, files.IngestDeps{Storage: storageSvc}), storageSvc, nil, nil, nil, nil, nil, nil)
