...
vi.mock("~/composables/useUploadQueue", () => ({
  useUploadQueue: () => ({
    ...
    erroredUploads: mocks.activeUploads.filter((u: UploadItem) => u.status === "error"),
...
describe("UploadProgress", () => {
  ...
  it("does not render anything when there are no active uploads", () => {
    const wrapper = mountComponent();
    expect(wrapper.text()).toBe("");
  });

  it("renders upload count, speed, and uploading progress", () => {
    mocks.activeUploads = [
      {
        id: "1",
        file: { name: "photo.jpg" },
