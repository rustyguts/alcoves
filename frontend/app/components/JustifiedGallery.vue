 ...
 * with sticky group headings and optional large section dividers. Purely
 * presentational — the parent owns data loading, grouping, and what `select`
 * does. Tracks its own width via a ResizeObserver so rows reflow on resize.
 */
...
<template>
  <div ref="rootEl">
    <template v-for="entry in laidOut" :key="entry.group.key">
        ...
        <div class="mt-1.5 flex flex-col" :style="{ gap: `${gap}px` }">
          <div v-for="(row, ri) in entry.rows" :key="ri" class="flex" :style="{ gap: `${gap}px` }">
            <button
              ...
              :key="box.item.id"
              type="button"
              class="group relative overflow-hidden rounded-[2px] bg-elevated focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:z-10"
