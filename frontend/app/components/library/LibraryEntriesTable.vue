  ...
  <table class="w-full">
    <thead class="sticky top-0 z-30 shadow-[0_1px_0_0] shadow-base-300/70">
      <tr>
        ...
        <th class="text-left text-xs font-medium text-muted px-4 py-3 bg-base-100/90 backdrop-blur-sm">
          Name
        </th>
        <th class="text-left text-xs font-medium text-muted px-4 py-3 bg-base-100/90 backdrop-blur-sm">
          Tags
        ...
        <tr class="cursor-pointer transition-colors" :class="[
          ...
          <td class="px-4 py-3 min-w-0">
            <div v-if="props.isRenaming(entry)" :data-rename-input-entry-id="entry.id">
              <input :value="renameValue" class="input input-sm w-full" autofocus
                @input="emit('updateRenameValue', ($event.target as HTMLInputElement).value)"
                @blur="emit('saveRename', entry)" @keydown.enter="emit('saveRename', entry)"
                @keydown.escape="emit('cancelRename')" @click.stop />
            </div>
