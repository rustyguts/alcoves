...
mockNuxtImport("useFetch", () => (urlFn: (() => string) | string) => {
...
describe("useLibraryExplorer", () => {
  ...
  it("fetchPage includes trashed param when in trash view", async () => {
    ...
    expect(mocks.fetch).toHaveBeenCalledWith("/api/libraries/lib-1/files", {
      query: { trashed: "true" },
    });
  });
  ...
  it("refreshTrashedCount fetches trashed count", async () => {
    mocks.fetch.mockResolvedValueOnce({ totalCount: 7 });
  ...
  it("refreshFolders fetches folders", async () => {
    ...
    mocks.fetch.mockResolvedValueOnce(folders);
