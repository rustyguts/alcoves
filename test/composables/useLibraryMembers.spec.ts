...
describe("useLibraryMembers", () => {
  ...
  it("inviteUserByEmail posts and refreshes on success", async () => {
    mocks.fetch.mockResolvedValueOnce({ action: "added" });
    ...
    expect(mocks.fetch).toHaveBeenCalledWith("/api/libraries/lib-1/users/invite-email", {
      method: "POST",
  ...
  it("inviteUserByEmail handles already_member", async () => {
    mocks.fetch.mockResolvedValueOnce({ action: "already_member" });
  ...
  it("inviteUserByEmail shows error on failure", async () => {
    mocks.fetch.mockRejectedValueOnce({ data: { message: "Bad request" } });
  ...
  it("createInviteLink posts and copies URL", async () => {
    ...
    mocks.fetch.mockResolvedValueOnce({ inviteUrl: "https://example.com/link" });

    const { createInviteLink } = create();
    await createInviteLink();

    expect(mocks.fetch).toHaveBeenCalledWith("/api/libraries/lib-1/users/invite-link", {
      method: "POST",
    });
    expect(refreshLibraryUsers).toHaveBeenCalled();
  });
  ...
  it("updateMemberRole patches role and refreshes", async () => {
    ...
    expect(mocks.fetch).toHaveBeenCalledWith("/api/libraries/lib-1/users/u1", {
      method: "PATCH",
      body: { role: "admin" },
    });
    expect(refreshLibraryUsers).toHaveBeenCalled();
  ...
  it("removeMember calls DELETE and refreshes", async () => {
    ...
    expect(mocks.fetch).toHaveBeenCalledWith("/api/libraries/lib-1/users/u1", { method: "DELETE" });
    expect(refreshLibraryUsers).toHaveBeenCalled();
  });
