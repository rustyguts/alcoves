...
- **CI (`.github/workflows/`)** — `ci.yml`: `backend-test` (`go test ./... -race -count=1 -p 1`), `unit-and-coverage` (lint + typecheck + vitest coverage), `e2e` (Playwright, 4-shard). `build-images.yml` (reusable) builds + pushes to GHCR. `publish.yml` calls it on push/tags/release. `release-please.yml` runs release-please and, on release, re-publishes `X.Y.Z`/`X.Y` tags. `website.yml` builds + deploys the Starlight site.
