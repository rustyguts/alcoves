...
- `live-documents.md` — Realtime collaborative markdown (Yjs CRDT); source editor + preview; roles; download roundtrip; MCP document tools.
...
- `frontend-architecture.md` — SSR topology, route groups, auth hooks, `createApi` + `/api` proxy, rune stores, shadcn-svelte, adapter-node.
...
- **Frontend** (`client/`): SvelteKit + Svelte 5 (runes) + shadcn-svelte (bits-ui primitives, zinc-neutral tokens, class-based dark via `@custom-variant`) + Tailwind 4 (CSS-first, `src/app.css`), built with `adapter-node`, run under Bun (`bun /app/build/index.js`). **Default SSR + hydration** app-wide — server `load`/`hooks.server.ts` fetch the **Go API** (never the DB) and forward the session cookie. `/s/[token]` share pages SSR for OG/SEO meta. A pre-hydration form guard in `app.html` blocks native `<form>` submits during the SSR→hydration window.
...
- **Seeded:** 3 users, 5 libraries (Family Photos, Travel 2025, Podcast Recordings, Alice's, Bob's — with member roles + face/object/sharing flags), nested folders, real image/video/audio files, two **live-document markdown files** (`trip-notes.md`/`packing-list.md` in Travel 2025 — plain `.md`, no CRDT rows; first open seeds the Y.Doc), tags, people + face crops, object detections, EXIF/GPS (Timeline + Map), transcripts, audio-event detections, waveforms, moments, a **public moment share** (`token="devseedshare01"`), highlight filters, the activity feed, app settings, a dev PAT.
...
| `docs` | Live Documents: opaque-Yjs-update log + snapshots (server never runs Yjs); `service.go` (GetState/Init/AppendUpdate/ListUpdates/Compact/ReplaceContent), `realtime.go` (per-file WS `Hub`/`Client` + Redis `doc:{fileId}` `Realtime` bus, mirrors `activity`). Materializes markdown to the file blob on compaction |
...
| `/api/libraries/:id/files/:fileId/doc*` | Live Documents: CRDT sync state, update append/replay, client-computed snapshot/compaction, and a dedicated per-document WebSocket (`document.go`). GET→viewer+, POST/PUT→admin+ via `LibraryAccessMiddleware`; WS is fan-out + inbound awareness only (no doc-write path) |
...
| `/api/mcp` | MCP HTTP transport (gated by `ALCOVES_MCP_HTTP_ENABLED`); authed by SDK `RequireBearerToken` (PAT or OAuth access token), not the global middleware |
| `/api/oauth/*` · `/.well-known/oauth-*` | MCP OAuth 2.1 authorization server: authorize/decision/token/register/revoke/connections + RFC 9728/8414 discovery (root). Gated by `ALCOVES_MCP_OAUTH_ENABLED` (`oauth_server.go`, `services/oauth`) |
| `/api/share/:token` · `/share/:token/video` · `/thumbnail` | Public moment share metadata + stream + thumbnail (no auth) (`share.go`) |

**Models & migrations:** all GORM entities in one file `internal/models/models.go`, UUID PKs with `BeforeCreate`. **No GORM soft-delete** — a nullable `TrashedAt *time.Time` on `File`/`Folder`/`Moment`, filtered manually. File sizes are `bigint`. Each async job type on `File` carries a `_status`/`_error`/`_version` + last-completed `_*ed_version` column set; a version mismatch triggers re-run (metadata + image_proxy use a 3-strike `_attempts` cap). Face embeddings are `vector(512)` (pgvector). Migrations are Goose SQL in `backend/migrations/` (`00001`→`00026`), embedded via `embed.go`, applied with `provider.Up()`; `00019` builds the HNSW `vector_cosine_ops` index (`m=16, ef_construction=64`, `CONCURRENTLY`/`NO TRANSACTION`); `00025` adds the MCP OAuth tables (`oauth_clients`/`oauth_authorization_codes`/`oauth_access_tokens`/`oauth_refresh_tokens`); `00026` adds Live Documents (`documents` 1:1 with a file — PK=`file_id`, `last_seq`/`snapshot`/`snapshot_seq` — + append-only `document_updates` PK`(file_id, seq)`, a dense per-doc sequence; both CASCADE from `files`).
...
- SvelteKit + Svelte 5 (runes forced for non-`node_modules` files in `svelte.config.js`) + shadcn-svelte (the Svelte 5 port of shadcn/ui, built on bits-ui) + Tailwind 4 + Bun. `adapter-node` with `envPrefix: 'FRONTEND_'` so the server reads `FRONTEND_*` and never collides with the Go API's `PORT` in the `all` role. Tailwind is CSS-first in `src/app.css` (`@import 'tailwindcss'` + `@import 'tw-animate-css'`; zinc-neutral `:root`/`.dark` design-token blocks, `--radius: 0.625rem`; no `tailwind.config`). shadcn-svelte primitives are vendored (not npm-installed) under `src/lib/components/ui/<name>/` (e.g. `ui/button/`, `ui/dialog/`, `ui/sidebar/`) — read the installed source directly rather than upstream docs for exact APIs; `@lucide/svelte` icons are used only inside these vendored primitives, offline/tree-shaken. `vite.config.ts` wires `@tailwindcss/vite` + `sveltekit()` (and the Vitest config).
...
- **`src/routes/` groups** (dynamic `[id]`/`[token]`/`[fileId]`/`[personId]`, catch-all `[...path]`):
  - `(app)/` — authed dashboard. `(app)/+layout.server.ts` redirects anon → `/login?redirect=…` and loads the sidebar libraries list (degrades to `[]`); `(app)/+layout.svelte` is the shadcn `Sidebar`-based shell. Sub-routes: `libraries/[id]` (browser; `trash/` sibling) + `feed`, `map`, `objects`, `tags`, `timeline`, `settings`, `people/[personId]`, `edit/[fileId]` (video editor, i.e. `libraries/[id]/edit/[fileId]`), `doc/[fileId]` (live-document editor, i.e. `libraries/[id]/doc/[fileId]`; `+page@(app).svelte` layout breakout like the video editor); plus `notifications`, `profile`, `search`, and owner-only `admin/` + `admin/jobs/` (gated by `admin/+layout.server.ts`, non-`owner` → `/`).
...
- **`src/lib/api/`** — `createApi(fetch)` factory (`client.ts`, 17 namespaces: `auth`, `libraries`, `files`, `folders`, `tags`, `highlightFilters`, `members`, `people`, `objects`, `downloads`, `search`, `invites`, `admin`, `moments`, `documents`, `meta`, `oauth`). Server `load` passes `event.fetch`; browser uses the `api` singleton (`index.ts`). `fetch.ts` is isomorphic `apiFetch` + `ApiError` (`.status`/`.data`); `url.ts` resolves data-vs-asset URLs: server keeps `/api/*` relative (so `handleFetch` rewrites + forwards the cookie); browser uses `PUBLIC_API_ORIGIN` when set (direct to Go, avoids Range mangling) else relative through the proxy.
- **`src/lib/state/`** — Svelte 5 rune stores (`*.svelte.ts`): `auth`, `theme`, `toast` (thin wrapper over `svelte-sonner`, rendered once in the root layout), `library-explorer`, `upload-queue` (TUS, `/api/tus`), `libraries-list`, `notifications`(+`-socket`), `async-job-status`, plus per-feature stores (`library-people/members/moments/feed/timeline/map/tags/folder-path/folder-actions/document-actions`, `transcript`/`transcribe-job`, `audio-detections`/`audio-detect-job`, `waveform*`, `highlight-filters`, `editor-highlights`/`editor-shortcuts`, `moment-downloads`, `download-zip`, `file-drop`).
- **`src/lib/collab/`** — Live Documents client: `doc-provider.svelte.ts` (the core — bridges `Y.Doc`↔backend: load/seed/init-409, debounced POST update queue, dedicated WS + seq-gap replay + poll fallback, y-protocols awareness, client-computed compaction, MCP-triggered `resync`; **Yjs/CM objects are NEVER `$state`** — reactivity via scalar counters), `base64.ts`, `user-colors.ts`.
- **`src/lib/components/`** — `ui/` holds both vendored shadcn-svelte primitives (`ui/<name>/`, e.g. `button/`, `card/`, `dialog/`, `dropdown-menu/`, `select/`, `sidebar/`, `sonner/` — upstream-maintained, not hand-edited beyond project theming) and Alcoves' own flat `ui/*.svelte` composites built on top of them (`AppIcon`, `AppModal`, `AppPanel`/`AppPanelRow`, `AlcovesImage`, `UserAvatar`, `ConfirmModal`, `AuthCardShell`, `OAuthGoogleButton`, `EmojiPicker`), feature dirs (`library/`, `editor/`, `doc/` — `MarkdownEditor` (CodeMirror 6 + `y-codemirror.next`, dynamic-imported like `LibraryMap`), `MarkdownPreview` (marked+DOMPurify), `DocEditorHeader`; `admin/`, `notifications/`, `profile/`), and top-level components (`LibraryHeader`, `LibraryBreadcrumb`, `LibrarySwitcher`, `SidebarLibraryNav`, `JustifiedGallery`, `FilePreview`, `TimelineScrubber`, `UploadModal`/`UploadProgress`, `LibraryMap` — Leaflet, browser-only). `src/lib/actions/portal.ts` is a `use:portal` action.
...
- ~2,100 unit tests. Coverage: V8, 90% (lines/functions/statements) + 80% branches in `vite.config.ts`; the per-file 60% floor is enforced separately (`scripts/coverage-floor.mjs`). Coverage-excluded (e2e-covered): `LibraryMap.svelte`, `editor/VideoEditorPlayer.svelte`, and the two trivial `libraries/*/+page.svelte` / `trash/+page.svelte` wrappers. Vendored shadcn-svelte primitives (`src/lib/components/ui/*/**`, e.g. `ui/button/`, `ui/dialog/`) are excluded from both the global coverage thresholds and the per-file floor by documented policy (upstream-maintained, exercised via composite tests + e2e, not hand-written) — see the matching `coverage.exclude` entry in `vite.config.ts` and the `isVendoredPrimitive` check in `scripts/coverage-floor.mjs`. Alcoves' own flat `ui/*.svelte` composites are NOT excluded and stay covered normally.
...
- MCP OAuth (custom-connector auth; `services/oauth`): `ALCOVES_MCP_OAUTH_ENABLED` (default false; requires HTTP transport + `ALCOVES_BASE_URL` as issuer), `_MCP_OAUTH_ACCESS_TTL`/`_REFRESH_TTL`/`_CODE_TTL` (`1h`/`720h`/`5m`), `_MCP_OAUTH_DCR_ENABLED` (default true), `_MCP_OAUTH_ALLOWED_REDIRECT_HOSTS` (optional allowlist)
...
- **Helm (`helm/alcoves/`)** — two `deploymentMode`s off the one image: `distributed` (default — `frontend` `web` :3000, `backend-api` `api` :3001, `backend-worker` `worker` no Service; RWX PVC) and `standalone` (one pod, `all` role; RWO-friendly for single-node clusters). Data PVC at `/app/data` for `local` storage, kept on uninstall (`helm.sh/resource-policy: keep`); chart does NOT deploy Postgres/Dragonfly. Shared env blocks in `_env.tpl`, fail-fast value validation in `_helpers.tpl` (rendered from NOTES.txt) + `values.schema.json`, per-workload PDB/HPA/scheduling knobs, optional MCP/Sentry/NetworkPolicy, `helm test` smoke pod, example values in `examples/`. CI lints + renders it (`helm-chart` job in `ci.yml`).
- **CI (`.github/workflows/`)** — `ci.yml`: `backend-test` (`go test -race -count=1 -p 1`, sharded 5 ways — each shard has its own postgres+dragonfly, `-p 1` stays within a shard; the heavy `internal/handlers` package is split by test name, the rest round-robined by `go list`; whisper.cpp cached), `unit-and-coverage` (in `client/`: `lint` + `typecheck` + `test:unit:coverage` + `coverage:floor`; installs the chromium browser for component tests), `e2e` (Playwright against the **REAL full stack** — `docker compose up -d --build --wait`, then `bunx playwright test` with `E2E_BASE_URL=http://localhost:3000`; **no mock backend**). `build-images.yml` (reusable) builds + pushes to GHCR (tags: `<short-sha>`, branch name, and — on a `main` push — a rolling `dev`); `publish.yml` calls it on push/tags/release and, after a `main` merge, refreshes a rolling **`dev` prerelease** (a single always-prerelease GitHub Release + `dev` git tag, never marked "Latest", for testing latest-main — separate from release-please's `v*` releases; skips release-please's own release commits); `release-please.yml` runs release-please + re-publishes `X.Y.Z`/`X.Y` tags on release; `website.yml` builds + deploys the Starlight site.
...
- **`helm/alcoves/Chart.yaml`** `appVersion` can drift from `VERSION` between releases (release-please bumps only `version:`) — pin `image.tag` rather than relying on the default image tag.
