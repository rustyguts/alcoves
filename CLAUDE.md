...
- **`src/routes/` groups** (dynamic `[id]`/`[token]`/`[fileId]`/`[personId]`, catch-all `[...path]`):
  - `(app)/` — authed dashboard. `(app)/+layout.server.ts` redirects anon → `/login?redirect=…` and loads the sidebar libraries list (degrades to `[]`); `(app)/+layout.svelte` is the shell. Sub-routes: `libraries/[id]` (browser; `trash/` sibling) + `feed`, `map`, `objects`, `tags`, `timeline`, `settings`, `people/[personId]`, `edit/[fileId]` (video editor), `doc/[fileId]` (live-document editor; `+page@(app).svelte` layout breakout like the video editor); plus `notifications`, `profile`, `search`, and owner-only `admin/` + `admin/jobs/` (gated by `admin/+layout.server.ts`, non-`owner` → `/`).
