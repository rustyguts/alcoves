/**
 * API response types — the mirror of the Go backend JSON contracts.
 *
 * Ported from the Nuxt app's `frontend/shared/types/api.ts`. Keep these in sync
 * with the backend handler structs. The `ActivityAction` union and
 * `image-variants.ts` MUST stay in lockstep with their backend counterparts.
 */
...
export interface AuthUser {
	...
	displayName: string;
	avatarUrl: string | null;
	/** Instance role (NOT the per-library role, which is `currentUserRole` on a library). */
	role: string;
}

/** A login session row (GET /api/auth/sessions). */
