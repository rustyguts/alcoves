...
import { jobStatusButton, type JobStatusLabels } from '$lib/utils/job-status-button';
...
describe('jobStatusButton', () => {
	...
	it('ignores progress when status is idle', () => {
		expect(jobStatusButton(null, 42, labels).label).toBe('Transcribe');
	});
	...
	it('returns warning + loading + disabled when processing', () => {
		expect(jobStatusButton('processing', null, labels)).toEqual({
			label: 'Transcribing…',
			color: 'warning',
			loading: true,
			disabled: true
	...
	it('treats undefined progress as no progress while in flight', () => {
		expect(jobStatusButton('processing', undefined, labels).label).toBe('Transcribing…');
	});
